...
class SyncZoomParticipantsJob implements ShouldQueue
{
    ...
    public function __construct(
        public ZoomSession $session,
        public bool $fallbackTriggered = false
    ) {}
        ...
        try {
            ...
            do {
                ...
                foreach ($res['participants'] ?? [] as $p) {
                    ...
                    $attendable = null;
                    if ($zoomUserId) {
                        $attendable = Student::where('zoom_user_id', $zoomUserId)->first()
                            ?: Instructor::where('zoom_user_id', $zoomUserId)->first();
                    }
                    if (! $attendable && ! empty($email)) {
                        $attendable = Student::where('email', $email)->first()
                            ?: Instructor::where('email', $email)->first();

                        // self-heal: update their zoom_user_id for future lookups
                        if ($attendable && $zoomUserId && empty($attendable->zoom_user_id)) {
                    ...
                    if ($dur !== null) {
                        $attendance->duration_minutes = (int) $dur;
                    }
            ...
            // Finalize attendance (mark absences, late, etc)
            $occurrence = $this->session->occurrence;
            if ($occurrence) {
                app(AttendanceService::class)->finalize($occurrence);
            }
            ...
            if ($this->fallbackTriggered) {
                ...
                if ($admin) {
                    Notification::send($admin, new WebhookFallbackDetected($this->session));
                }
            }
        } catch (\Throwable $e) {
            ...
            if (method_exists($e, 'getCode') && $e->getCode() === 429) {
                throw $e; // rethrow to trigger retry/backoff
            }
    ...
    public function failed(\Throwable $exception)
    {
        ...
        $admin = User::where('user_type', 'admin')->first();
        if ($admin) {
            Notification::send($admin, new JobFailedNotification(
