                    ...
                    <form action="{{ route('admin.courses.update', $course) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" class="form-control @error('subtitle') is-invalid @enderror" id="subtitle" name="subtitle" value="{{ old('subtitle', $course->subtitle) }}">
                                @error('subtitle')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select select2-facilitators @error('facilitator_ids') is-invalid @enderror" id="facilitator_ids" name="facilitator_ids[]" multiple>
                                    @foreach($facilitators as $facilitator)
                                        <option value="{{ $facilitator->id }}" @selected($course->facilitators->pluck('id')->contains($facilitator->id))>
                                            {{ $facilitator->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('course_hours') is-invalid @enderror" id="course_hours" name="course_hours" value="{{ old('course_hours', $course->course_hours) }}" min="1">
                                @error('course_hours')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" class="form-control @error('fee') is-invalid @enderror" id="fee" name="fee" value="{{ old('fee', $course->fee) }}" step="0.01" min="0" required>
                                @error('fee')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-select @error('currency') is-invalid @enderror" id="currency" name="currency" required>
                                    ...
                                    <option value="NGN" @selected(old('currency', $course->currency) == 'NGN')>NGN (₦)</option>
                                    <option value="USD" @selected(old('currency', $course->currency) == 'USD')>USD ($)</option>
                                    <option value="GBP" @selected(old('currency', $course->currency) == 'GBP')>GBP (£)</option>
                                    <option value="EUR" @selected(old('currency', $course->currency) == 'EUR')>EUR (€)</option>
                                </select>
                                @error('currency')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" class="form-control @error('max_enrollees') is-invalid @enderror" id="max_enrollees" name="max_enrollees" value="{{ old('max_enrollees', $course->max_enrollees) }}" min="1">
                                @error('max_enrollees')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            @if($course->featured_image)
                                <div class="mb-2">
                                    <img id="imagePreview" src="{{ asset($course->featured_image) }}" alt="{{ $course->title }}" class="img-thumbnail" style="max-width: 200px; max-height: 200px;">
                                </div>
                            @else
                            ...
                            <small class="form-text text-muted">Max 2MB. Accepted formats: jpg, jpeg, png, gif. Leave empty to keep current image.</small>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_online" name="is_online" value="1" @checked(old('is_online', $course->is_online))>
                            ...
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_offline" name="is_offline" value="1" @checked(old('is_offline', $course->is_offline))>
                            ...
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_featured" name="is_featured" value="1" @checked(old('is_featured', $course->is_featured))>
                            ...
                            <div class="col-md-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" @checked(old('is_active', $course->is_active))>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <div class="form-check form-switch p-3 bg-light border rounded">
                                    <input class="form-check-input" type="checkbox" id="is_free" name="is_free" value="1" @checked(old('is_free', $course->is_free)) onchange="toggleFeeFields()">
                            ...
                            <div class="card-body">
                                <div id="courseDatesContainer">
                                    @if($course->courseDates->count() > 0)
                                        @foreach($course->courseDates->sortBy('sequence') as $index => $date)
                                            @include('admin.courses.partials.date-form-edit', ['index' => $index, 'date' => $date])
                                        @endforeach
                                    @else
...
<script>
        ...
        if (file) {
            ...
            reader.onload = function(event) {
                ...
                if (!container) {
                    ...
                    document.getElementById('featured_image').parentElement.insertBefore(container, document.getElementById('featured_image'));
    ...
    let dateIndex = {{ $course->courseDates()->count() }};
    let venueIndex = {};

    // Initialize venueIndex for existing dates
    @foreach($course->courseDates()->get() as $index => $date)
        venueIndex[{{ $index }}] = {{ $date->venues()->count() }};
    @endforeach
